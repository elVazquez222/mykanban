// Die <Task /> Komponente hat nicht funktioniert, weil die events nicht bei den Funtionen in der 
// <App /> Konponete angekommen sind. "Cannot read propertie pageX of Null" in Bezug auf "let newPosX = event.pageX"

import React from 'react'


export default function Task(props) {
  return (

    <div
      id={props.id}
      key={props.id}
      className={`task `}
      draggable="true"
      // onDrag={event => props.dragging(event, props.id)}
      // onDragEnd={event => { props.setNewPosition(event, props.id) }}
      // onMouseDown={event => willDrag(event, task.id)}
      style={{ left: props.left, top: props.top }}
    >
      <div className="task-header">
        <span className="task-Title">{`Task 0${props.id}`} </span>
        <div className="task-Developer">
          <img className="developer-picture" alt="Avatar" src="data:image/jpeg;base64," />
          <span className="developer-name">{props.developer}</span>
          <span className="details-expand-btn" onClick={() => props.detail_toggle(`details_${props.id}`)} >i</span>
        </div>
      </div>
      <div className={`task-Details`} id={`details_${props.id}`}>
        <span className="d-o-d-title">
          Definition of Done:
                  </span>
        <span className="d-o-d-list">
          {props.dod}
        </span>
      </div>
    </div>

  )
}
